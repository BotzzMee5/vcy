...
`.trim()
    ...
    */
     ...
     * @param {Object} options 
     */
    conn.sendButtonLoc = async (jid, buffer, content, footer, row1, quoted, options = {}) => {
...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/a6ac2251f2187440365cb.jpg')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'BotzzMee', body: wm, sourceUrl: 'https://chat.whatsapp.com/JavkfMp03vq1V79cQnPuFv', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
